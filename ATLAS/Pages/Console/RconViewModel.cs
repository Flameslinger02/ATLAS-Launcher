...
/// <summary>
/// Live BattlEye RCon management for the active profile. Three sub-tabs — Players (grid + context menu +
/// 30s auto-refresh), Bans (list + unban/manual-ban/export) and the RCON command console (history + quick
/// buttons). Bans are written to <c>BanHistory</c>; player join/leave is derived from the 30s poll diff and
/// written to <c>PlayerHistory</c>. Singleton VM (holds long-lived RCON/server subscriptions; mirrors the
/// Dashboard no-leak rule). Server/ATLAS logs and the updater live on the separate Console page.
/// </summary>
public partial class RconViewModel : BaseViewModel, IDisposable
{
    ...
    public RconViewModel(IBattlEyeRconClient rcon, IProfileService profiles, IDialogService dialogs, AtlasDatabase db)
    {
        _rcon = rcon;
        _profiles = profiles;
        _dialogs = dialogs;
    ...
    private async Task Connect()
    {
        ...
        {
            await _dialogs.ShowErrorAsync("RCON", "This profile has no RCON password. Set one on the profile's Network tab.");
            return;
        }
